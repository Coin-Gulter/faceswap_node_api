...
from node.task_manage import TaskManager
from node.cdn_manager import CDN
...
def post_face_swap(request):
    ...
    try:
        ...
        decode_img_path = f"{settings.IMAGES_PATH + task_id + '/'}" 
        ...
        try:
            ...
            return response.Response(
                {'error': "Invalid base64 encoding for face images"},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            source_extension = str(os.path.splitext(source)[1])
            ...
            if serialized_task.is_valid():
                ...
                return response.Response(responce_data, status=status.HTTP_200_OK)
...
def get_faceswap_result(request):
    ...
    output_file_path = f'{settings.CDN_UPLOAD_PATH}/data/result/{str(task_id)}/{os.path.basename(source)}'
...
def post_template(request):
    ...
    premium = False if request.POST.get('premium') == '0' else True
    ...
    if file:
        ...
        decode_source_path =  f"{settings.SOURCE_PATH + str(sort_id) + file_extension}"
    ...
    CDN_OBJ.upload_to_cdn(decode_source_path)

    if thumb_file:
        file_extension = os.path.splitext(str(thumb_file))[1]
        decode_thumb_path =  f"{settings.THUMB_PATH + str(sort_id) + file_extension}"

        with open(decode_thumb_path, 'wb') as destination:
            for f in thumb_file:
                destination.write(f)

        CDN_OBJ.upload_to_cdn( decode_thumb_path)
        template_model_data['thumb'] = decode_thumb_path
        task_publish_data['thumb'] = decode_thumb_path

    if preview_source_file:
        file_extension = os.path.splitext(str(preview_source_file))[1]
        decode_preview_source_path =  f"{settings.PREVIEW_SOURCE_PATH + str(sort_id) + file_extension}"

        with open(decode_preview_source_path, 'wb') as destination:
            for f in preview_source_file:
                destination.write(f)

        CDN_OBJ.upload_to_cdn( decode_preview_source_path)
        template_model_data['preview_source'] = decode_preview_source_path
